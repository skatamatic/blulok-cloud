...
- **Contact-identity collisions**: a facility user can map to only one FMS tenant. If a new FMS tenant shares email/phone with a user already mapped to a different FMS id (HQ testers sharing `+12504882375`), detection emits an **invalid** `tenant_added` for review instead of inserting a duplicate `fms_entity_mappings` row (that unique constraint previously 500’d the whole sync). Several FMS tenants that collide on the **same** BluLok user become **one** review row (`after_data.collidingExternalIds` lists every FMS id so occupied-unit checks still skip those tenants).
