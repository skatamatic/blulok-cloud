...
- **Apply failure toasts**: when apply partially or fully fails, toasts summarize counts by failure reason (e.g. “26 unit updates failed: 22 can’t change status while tenants are assigned…”) using structured `errorDetails` from the apply API—no raw UUIDs or change_type dumps. Failed rows stay in the review modal for inspection.
