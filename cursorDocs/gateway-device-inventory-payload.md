...
| Flag | Meaning |
|------|---------|
| `metadata.createdFromGatewaySync` | `true` when the gateway has reported this device (auto-provision **or** later inventory match on a manual row). Alone does **not** make a row sync-managed if `manuallyAdded` is also true. |
| `metadata.manuallyAdded` | Set `true` on admin UI / REST create (with `createdFromGatewaySync: false`). Never removed by gateway delta. Stays true after gateway sees the device. |
