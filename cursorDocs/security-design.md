...
- Flow D (Revocation): Cloud pushes signed Denylist Update Command to Gateway to target locks. When **`GATEWAY_DENYLIST_SYNC_ENABLED`** is on (default **off**), active-gateway reconnect (`AUTH_OK`) also pushes a full **`DENYLIST_SYNC`** replace snapshot (all operational devices) so restarted gateways reconcile local denylist state without waiting for inventory sync.
