...
- **Invite SMS / email**: When `sendInvite: true` and no password was set, the server calls `FirstTimeUserService.sendInvite` after create. Delivery goes to **every enabled channel** that has a contact on the user (SMS when `phone_number` is set and SMS is enabled; email when `email` is set and email is enabled). Clients may omit `sendInvite` or use **Resend invite** on the user profile later. FMS-created tenants follow per-facility **`invitePolicy`** (default `none`) — see `fms-webhooks.md`.
