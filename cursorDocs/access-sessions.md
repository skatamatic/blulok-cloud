...
8. **Lock-state echo** — firmware sometimes posts `POST /access-events` with `action: access_granted`, `method: mobile_key`, `metadata.source: gateway_lock_state`, and `metadata.event: none` (no `user_id`). That is a physical lock report, not a credential eval. Cloud **does not** open a new session. It settles a live pending/open on that device (same as a `devices/state` lock), links a raw `lock`/`unlock` activity (`metadata.lock_state_echo`), and if nothing is live still **persists the activity** so the payload is not dropped. A real grant (`event: granted` and/or `actor.user_id`) is unchanged.
