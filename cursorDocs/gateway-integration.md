...
| **Grace period** | During **`GATEWAY_OFFLINE_GRACE_MS`** (default **20s**), product liveness (`GatewayEventsService.getFacilityProductLiveness()`) stays **connected** so UI pills and device reachability do **not** flap. Raw transport status (`getFacilityConnectionStatus()`) still reports the socket as down for command routing. After the grace window, the disconnecting instance re-reads **`gateways.last_seen`**: if AUTH on **any** instance refreshed it after the disconnect, offline persist + **Gateway Offline** alerts are skipped (Cloud Run multi-instance). Only when `last_seen` is still older than the disconnect does DB `gateways.status` flip offline. Dev/e2e can temporarily override via **`PUT /api/v1/dev/gateway-offline-grace`** `{ "grace_ms": <ms\|null> }` (ADMIN / DEV_ADMIN; process-local; `null` restores default). |
...
- **Offline grace (`GATEWAY_OFFLINE_GRACE_MS`, default 20s):** inbound `/ws/gateway` disconnects **do not** immediately persist `gateways.status = offline`, fan out **Gateway Offline** in-app alerts, or flip product-facing **`connected`** / device reachability. The backend waits for reconnect. The grace timer is **process-local** (the instance that saw the close). When it fires, confirm against shared DB: **`gateways.last_seen` newer than the disconnect** (AUTH on this or another Cloud Run instance) → skip offline persist and alerts. Stale `last_seen` + no local socket → persist offline and notify. **`getFacilityProductLiveness()`** (used by `gateway_status_update` and device reachability enrichment) stays `connected: true` during grace; **`getFacilityConnectionStatus()`** remains the raw socket signal for commands/firmware. Dashboard toasts fire when the broadcast reports confirmed offline (no second client-side debounce). For e2e/local speed-ups, **`PUT /api/v1/dev/gateway-offline-grace`** `{ "grace_ms": <0–120000|null> }` overrides the process value (`null` clears).

## Google Cloud Run–specific issues

### 1. Multiple instances and in-memory gateway state

`WebsocketGatewayTransport` keeps **connected gateways in process memory** (`facilityToClient`). If Cloud Run scales to **more than one instance**, a command or debug stream might hit an instance where **that facility is not connected**. Hourly Cloud Run `--timeout=3600` closes are expected; AUTH typically lands in 1–6s. Offline **notifications** must not follow that recycle: the instance that lost the socket confirms against **`gateways.last_seen`** before persisting offline (see Offline grace above). Command routing is still in-memory — session affinity / `--max-instances=1` / a shared bus remain the mitigations for sending commands to the instance that holds the socket.
...
**Separate from manual sync:** the **Sync** tab’s **`POST /gateways/:id/sync`** outbound pull (UI: `GatewayManualSyncPanel`) is not recorded in this audit trail (and is blocked during gateway recovery, same as inventory sync).
