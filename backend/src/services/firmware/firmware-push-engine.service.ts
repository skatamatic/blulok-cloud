 ...
 *
 * Handles firmware push execution (v1 chunked and v2 signed-URL), chunk ACK
 * handling, progress reports, disconnect/reconnect resume, and verification
 * timeouts. Manages in-memory push state via firmware-push-session.store.
 *
 * Extracted from FirmwareService to isolate push runtime from catalog CRUD.
 */

import * as crypto from 'crypto';
import { v4 as uuidv4 } from 'uuid';
import { FirmwareModel, FirmwareImage, FirmwareTargetType } from '@/models/firmware.model';
...
import { getFirmwareStorageProvider } from './firmware-storage.factory';
import { logger } from '@/utils/logger';
import { FIRMWARE_CHUNK_SIZE_BYTES } from '@/constants/firmware-chunk.constants';
import {
  activePushes,
  verifyingTimeouts,
  transferDisconnectTimeouts,
  v2TransferDeadlines,
  resumeInFlightPushes,
  resumeFacilityRetryTimers,
  resumeFacilityRunsInFlight,
  ActivePush,
...
export class FirmwarePushEngineService {
  ...
  // =========================================================================
  // Update Status Handling (called by WS transport)
  // =========================================================================
  ...
  // =========================================================================
  // Progress Handling (called by WS transport)
  // =========================================================================
  ...
  /**
   * Handle a gateway facility disconnection.
   */
  ...
  static async recoverInFlightStateOnStartup(): Promise<void> {
    const allActivePushes = await this.pushModel.findAllActive();
    if (allActivePushes.length === 0) {
      return;
    }

    const now = Date.now();
    for (const push of allActivePushes) {
      if (push.status === 'transferring') {
