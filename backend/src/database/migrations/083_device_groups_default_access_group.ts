...
export async function up(knex: Knex): Promise<void> {
  const hasColumn = await knex.schema.hasColumn('device_groups', 'is_default');
  if (!hasColumn) {
    await knex.schema.alterTable('device_groups', (table) => {
      table.boolean('is_default').notNullable().defaultTo(false).after('is_global_shared');
    });
  }
  ...
  if (hasIndex) {
    const indexes = await knex.raw(`
      SHOW INDEX FROM device_groups WHERE Key_name = 'idx_device_groups_facility_default'
    `);
    const rows = indexes?.[0] ?? [];
    if (!Array.isArray(rows) || rows.length === 0) {
      await knex.schema.alterTable('device_groups', (table) => {
        table.index(['facility_id', 'is_default'], 'idx_device_groups_facility_default');
      });
    }
  }
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.alterTable('device_groups', (table) => {
    table.dropIndex(['facility_id', 'is_default'], 'idx_device_groups_facility_default');
