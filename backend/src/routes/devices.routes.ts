...
import { assertJwtFacilityClaim, hasJwtFacilityClaim } from '../utils/facility-access-claim.utils';
...
import { registerDeviceLockCommandRoutes } from './devices-lock-commands.routes';
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (AuthService.isFacilityScoped(user.role)) {
        ...
        if (!gatewayRow || !hasJwtFacilityClaim(user, gatewayRow.facility_id)) {
...
registerGet(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!assertJwtFacilityClaim(res, user, device.facility_id, 'Access denied to this device')) {
        return;
      }
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (user.role === UserRole.FACILITY_ADMIN) {
        if (!assertJwtFacilityClaim(res, user, facilityId, 'Access denied to this facility')) {
          return;
        }
      }
...
registerPut(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (!assertJwtFacilityClaim(res, user, existing.facility_id, 'Access denied to this facility')) {
      return;
    }
...
registerPut(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (
      facilityId &&
      !assertJwtFacilityClaim(res, user, facilityId, 'Access denied to this facility')
    ) {
      return;
    }
...
// Lock command routes (BluLok lock/unlock, occupied unit override, access control lock/unlock)
registerDeviceLockCommandRoutes(router, deviceModel);
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (AuthService.isFacilityAdmin(user.role)) {
        ...
        if (!device || !hasJwtFacilityClaim(user, device.facility_id)) {
          res.status(403).json({
            success: false,
