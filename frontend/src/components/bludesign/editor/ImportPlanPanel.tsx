/**
 * Editor import plan panel body — vector overlay with optional source image toggle.
 * Rendered inside {@link FloatingPanel} / dock chrome from EditorCanvas.
 */
...
export interface ImportPlanPanelContentProps {
  facilityId: string;
  layoutImport: LayoutImportMetadata;
  width: number;
  height?: number;
  /** Synced from editor 3D selection (placed object ids). */
...
export const ImportPlanPanelContent: React.FC<ImportPlanPanelContentProps> = ({
  facilityId,
  layoutImport,
  width,
  height,
  ...
  useEffect(() => {
    ...
    if (id) viewerRef.current?.focusUnit(id);
  }, [selectedIds]);
  ...
  }, [facilityId, reloadKey, loadImage, showImage]);
  ...
  return (
    <div
      ...
      style={{ width, ...(height !== undefined ? { height } : undefined) }}
    >
      <div
...
/** @deprecated Use ImportPlanPanelContent — panel chrome lives in EditorCanvas. */
export const ImportPlanPanel = ImportPlanPanelContent;

export default ImportPlanPanelContent;
