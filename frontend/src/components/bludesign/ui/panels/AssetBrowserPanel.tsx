...
import { isAutoGeneratedAsset } from '../../assets/autoGenerated';
import { AutoGeneratedAssetFolder } from '../AutoGeneratedAssetFolder';
...
export const AssetBrowserPanel: React.FC<AssetBrowserPanelProps> = ({
  ...
  // Auto-generated (layout-import) assets are pulled out into a dedicated,
  // collapsible folder so they never clutter the normal category lists.
  ...
  const groupedAssets = shouldGroup 
    ? normalAssets.reduce((groups, asset: AssetMetadata) => {
  ...
  const renderAutoAssetCard = (asset: AssetMetadata) => (
    <AssetCard
      key={asset.id}
      asset={asset}
  ...
  const autoGeneratedFolder = autoAssets.length > 0 && (
    <AutoGeneratedAssetFolder
      ...
      isDark={isDark}
      layout="compact"
      cardWidth={ASSET_CARD_SIZE}
      cardHeight={ASSET_CARD_SIZE + 12}
      columns={columns}
      gap={ASSET_GRID_GAP}
    >
      {autoAssets.map(renderAutoAssetCard)}
    </AutoGeneratedAssetFolder>
  );
      ...
      <div className="space-y-4">
        {shouldGroup ? (
          ...
          <div className="grid" style={gridStyle}>
            {normalAssets.map((asset: AssetMetadata) => (
              <AssetCard
                key={asset.id}
                asset={asset}
                isActive={activeAssetId === asset.id}
        ...
        {shouldGroup && autoAssets.length > 0 && (
          <div className="grid" style={gridStyle}>
            {autoGeneratedFolder}
          </div>
        )}
