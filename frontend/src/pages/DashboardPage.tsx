...
export default function DashboardPage() {
  ...
  // Manage widget subscriptions based on visible widgets (stats and units, layout sync disabled)
  useEffect(() => {
    // Managing subscriptions for widgets
    ...
    const visibleBatteryWidgets = widgetInstances.filter(widget => 
      batteryWidgetTypes.includes(widget.type)
    );
    ...
    if (visibleStatsWidgets.length > 0) {
      // Managing subscriptions for stats widgets
      ...
      // No stats widgets visible, unsubscribing from general_stats
      widgetSubscriptionManager.unsubscribe('general_stats');
    }
    ...
    if (visibleUnitsWidgets.length > 0) {
      ...
      // No units widgets visible, unsubscribing from units
      widgetSubscriptionManager.unsubscribe('units');
    }

    if (visibleBatteryWidgets.length > 0) {
      // Managing subscriptions for battery widgets
      subscriptions.push('battery_status');
      subscriptionMap['battery_status'] = {
        handler: (data: any) => {
          // Battery data received - individual widgets handle their own data
          console.log('🔋 Dashboard: Received battery data:', data);
        },
        errorHandler: (error: string) => console.error('Battery subscription error:', error)
      };
    } else {
      // No battery widgets visible, unsubscribing from battery_status
    ...
    return () => {
      // Component unmounting, cleaning up subscriptions
  ...
  useEffect(() => {
    const loadWidgetLayout = async () => {
      try {
        // Load from API instead of localStorage to ensure we get the latest data
  ...
  const addWidget = useCallback(async (widgetType: string) => {
    ...
    try {
      ...
      if (widgetType.startsWith('stats-')) {
        // For stats widgets, use the old pattern
        baseId = widgetType.replace('stats-', '').replace('-', '') + '_stats';
      } else {
        // For other widgets, use the widget type as-is
        baseId = widgetType;
      }
