...
export default function AddFacilityPage() {
  ...
  const validateGateway = (): boolean => {
    const newErrors: Record<string, string> = {};
    if (!gatewayData.name.trim()) newErrors.gateway_name = 'Gateway name is required';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!facilityResponse?.facility?.id) {
        setErrors({ submit: 'Failed to create facility. Invalid response from server.' });
        return;
      }

      if (step === 'gateway') {
        await apiService.createGateway({
          facility_id: facilityResponse.facility.id,
          ...gatewayData,
          status: 'offline',
        });
      }
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`block w-full pl-10 pr-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 ${
                        errors.gateway_name ? 'border-red-300 dark:border-red-600' : 'border-gray-300 dark:border-gray-600'
                      } bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`block w-full pl-10 pr-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 ${
                        errors.ip_address ? 'border-red-300 dark:border-red-600' : 'border-gray-300 dark:border-gray-600'
                      } bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
                    />
                  </div>
                </div>
        ...
        <div className="px-8 py-6 border-t border-gray-200 dark:border-gray-700 flex justify-between">
          ...
          <div className="flex space-x-3">
            {step === 'facility' ? (
              <button
                onClick={() => validateForm() && setStep('gateway')}
