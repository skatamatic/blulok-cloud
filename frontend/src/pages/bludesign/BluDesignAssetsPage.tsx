...
export default function BluDesignAssetsPage() {
  ...
  const confirmDeleteAllAuto = useCallback(async () => {
    ...
    if (failed.length > 0) {
      alert(
        `Deleted ${targets.length - failed.length} of ${targets.length} assets. Failed: ${failed.join(', ')}`
      );
    }
  }, [autoAssets, isBuiltInAsset, removeAssetsFromLocalState]);
