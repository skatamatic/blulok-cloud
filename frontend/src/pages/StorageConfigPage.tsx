 ...
 *
 * Admin view for configuring the system-wide firmware storage backend.
 * Supports Local, Google Cloud Storage, and Google Drive providers with
 * a write/read/delete test cycle that reports per-step results.
 */
...
const StorageConfigPage: React.FC = () => {
  ...
  const handleTest = async () => {
    ...
    try {
      const data = await apiService.post('/admin/storage-config/test', {
        providerType,
        providerConfig: buildPayload(),
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const data = await apiService.put('/admin/storage-config', {
        providerType,
        providerConfig: buildPayload(),
      });
      if (data.success) {
  ...
  return (
    <div className={`min-h-full p-6 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Configure where firmware files and system data are stored
            </p>
          </div>
        </div>
        ...
        {configSource && (
          <div className={`
            inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium mb-6
        ...
        `}>
          ...
          <p className={`text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Choose where firmware binaries will be stored. The test button performs a full
            write, read-back, and delete cycle to verify connectivity.
          </p>
          ...
          <div className="space-y-4 mb-6">
            {providerType === 'local' && (
              <div>
                ...
                <input
                  type="text"
                  value={providerConfig.basePath ?? ''}
                  onChange={(e) => updateConfig({ basePath: e.target.value })}
                  placeholder="./storage/firmware"
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={providerConfig.bucketName ?? ''}
                      onChange={(e) => updateConfig({ bucketName: e.target.value })}
                      placeholder="blulok-firmware"
                      className={inputClass}
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={providerConfig.keyFilePath ?? ''}
                    onChange={(e) => updateConfig({ keyFilePath: e.target.value })}
                    placeholder="/path/to/service-account-key.json"
                    className={inputClass}
                  />
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                    Leave empty to use default credentials or key file contents below
                  </p>
                </div>
                <div>
                  <label className={labelClass}>Service Account Key File Contents</label>
                  <textarea
                    value={providerConfig.keyFileContents ?? ''}
                    onChange={(e) => updateConfig({ keyFileContents: e.target.value })}
            ...
            {providerType === 'gdrive' && (
              <>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={providerConfig.rootFolderId ?? ''}
                    onChange={(e) => updateConfig({ rootFolderId: e.target.value })}
                    placeholder="1AbC2dEf3GhI4jKl5MnOp6QrSt7UvWx8Yz"
                    className={inputClass}
                  />
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                    Get this from the folder URL: drive.google.com/drive/folders/<strong>[FOLDER_ID]</strong>
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                `}>
                  ...
                  `}>
                    {testSuccess
                      ...
                      : <><ExclamationCircleIcon className="w-4 h-4" /> {testError}</>
                    }
                  </div>
                  {/* Steps */}
                    ...
                    {testSteps.map((step, i) => (
                      <div
                        ...
                        className={`
                          ...
                          ${i > 0 ? (isDark ? 'border-t border-gray-800' : 'border-t border-gray-100') : ''}
                        `}
                      >
          ...
          <AnimatePresence>
            {saveError && (
              <motion.div
                ...
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                className="mb-4 flex items-center gap-2 text-red-500 text-sm"
              >
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`
                  ...
                  transition-all duration-150
                  ${isTesting ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90'}
                  ${isDark
